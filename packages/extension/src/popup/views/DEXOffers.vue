...
<template>
  <div class="flex flex-col h-full">
    <div class="flex items-center gap-2 px-4 py-3 border-b border-border">
      <button class="p-1 rounded hover:bg-bg-hover" @click="router.push('/explore/dex')">
    ...
    >
      <div class="text-center">
        <p class="text-sm text-text-muted">No open offers</p>
        <p class="text-xs text-text-muted mt-1">Place an order on the DEX to see it here</p>
      </div>
    </div>
      ...
      <div v-for="offer in dexStore.accountOffers" :key="offer.seq" class="px-4 py-3">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xs text-text-muted">for {{ formatAmount(offer.takerGets) }}</p>
          </div>
