...
<template>
  <div class="flex items-center justify-between py-2 px-3 border-b border-border last:border-0">
    <div class="flex-1 min-w-0">
      <p class="text-sm font-medium">{{ formatAmount(props.offer.amount) }}</p>
      <p class="text-xs text-text-muted truncate">{{ truncateAddress(props.offer.owner) }}</p>
    </div>
